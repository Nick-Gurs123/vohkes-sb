...
ctypes.windll.kernel32.SetConsoleTitleW(f'[VohkesSB Selfbot v{SELFBOT.__version__}] | Loading...')
...
def startprint():
    ...
    |::|    |              /:::/    / \:::\    \     /::::\    \              /::::\    \              /::::\   \:::\    \       \:::\   \:::\    \               \:::\   \:::\    \      /::::\   \:::\    \   
    |::|    |     _____   /:::/____/   \:::\____\   /::::::\    \   _____    /::::::\____\________    /::::::\   \:::\    \    ___\:::\   \:::\    \            ___\:::\   \:::\    \    /::::::\   \:::\    \  
    |::|    |    /\    \ |:::|    |     |:::|    | /:::/\:::\    \ /\    \  /:::/\:::::::::::\    \  /:::/\:::\   \:::\    \  /\   \:::\   \:::\    \          /\   \:::\   \:::\    \  /:::/\:::\   \:::\ ___\ 
    ...
    \::::::::::/____/         \::::::::/    /                /:::/    /           |::|   |              \:::\   \::/    /        \:::\  /:::/    /                \:::\  /:::/    /        \:::\  /:::/    /    
     ~~~~~~~~~~                \::::::/    /                /:::/    /            |::|   |               \:::\   \/____/          \:::\/:::/    /                  \:::\/:::/    /          \:::\/:::/    /     
                       ...
                       {Fore.CYAN}VohkesSB {SELFBOT.__version__} | {Fore.GREEN}Logged in as: {VohkesSB.user.name}#{VohkesSB.user.discriminator} {Fore.CYAN}| ID: {Fore.GREEN}{VohkesSB.user.id}   
...
def Init():
    if config.get('token') == "token-here":
        ...
        token = config.get('token')
        try:
            VohkesSB.run(token, bot=False, reconnect=True)
            os.system(f'title (VohkesSB Selfbot) - Version {SELFBOT.__version__}')
...
colorama.init()
VohkesSB = discord.Client()
VohkesSB = commands.Bot(
    description='VohkesSB Selfbot',
    command_prefix=prefix,
    self_bot=True
)
VohkesSB.remove_command('help') 
...
async def btc_status():
    ...
    await VohkesSB.change_presence(activity=btc_stream)
...
async def on_message_edit(before, after):
    await VohkesSB.process_commands(after)
...
async def on_message(message):
    ...
    if f'Congratulations <@{VohkesSB.user.id}>' in message.content:
        if giveaway_sniper == True:
            if message.author.id == 294882584201003009:    
                print(""
    ...
    if 'privnote.com' in message.content:
        if privnote_sniper == True:
            ...
            return
    await VohkesSB.process_commands(message)
...
async def on_connect():
    ...
    startprint()
    ctypes.windll.kernel32.SetConsoleTitleW(f'[VohkesSB Selfbot v{SELFBOT.__version__}] | Logged in as {VohkesSB.user.name}')
                      ...
                      module.prototype._connect = function(url) {
                        ...
                        this._handleDispatch = function(data, type) {
                          if (type === 'READY') {
                            console.log(data);
                            data.user.bot = false;
                            data.user.email = 'VohkesSB-Was-Here@Fuckyou.com';
...
@VohkesSB.command(aliases=['shorteen'])
...
@VohkesSB.command(name='ebay-view', aliases=['ebay-view-bot', 'ebayviewbot', 'ebayview'])
...
@VohkesSB.command(aliases=['geolocate', 'iptogeo', 'iptolocation', 'ip2geo', 'ip'])
...
@VohkesSB.command(aliases=['pfp', 'avatar'])
...
@VohkesSB.command(aliases=['ri', 'role'])
...
@VohkesSB.command(name='1337-speak', aliases=['1337speak'])
...
async def blank(ctx): # b'\xfc'
    ...
    if config.get('password') == 'password-here':
        ...
        with open('Images/Avatars/Transparent.png', 'rb') as f:
          try:      
             await VohkesSB.user.edit(password=password, username="ٴٴٴٴ", avatar=f.read())
          except discord.HTTPException as e:
            print(f"{Fore.RED}[ERROR]: {Fore.YELLOW}{e}"+Fore.RESET)

@VohkesSB.command(aliases=['pfpget', 'stealpfp'])
    ...
    if config.get('password') == 'password-here':
        ...
        try:
            ...
            with open('Images/Avatars/Stolen/Stolen.png', 'rb') as f:
              await VohkesSB.user.edit(password=password, avatar=f.read())
        except discord.HTTPException as e:
            print(f"{Fore.RED}[ERROR]: {Fore.YELLOW}{e}"+Fore.RESET)

@VohkesSB.command(name='set-pfp', aliases=['setpfp', 'pfpset'])
...
@VohkesSB.command(aliases=['dong', 'penis'])
...
@VohkesSB.command(aliases=['changehypesquad'])
...
@VohkesSB.command(aliases=['tokenfucker', 'disable', 'crash']) 
...
@VohkesSB.command(aliases=['fakeconnection', 'spoofconnection'])
...
@VohkesSB.command(aliases=['tokinfo', 'tdox'])
...
async def copy(ctx): # b'\xfc'
    await ctx.message.delete()
    await VohkesSB.create_guild(f'backup-{ctx.guild.name}')
    await asyncio.sleep(4)
    for g in VohkesSB.guilds:
...
async def destroy(ctx): # b'\xfc'
    ...
    try:
        await ctx.guild.edit(
            name=RandString(),
            description="https://VohkesSB.wtf",
            reason="https://VohkesSB-selfbot.github.io",
            icon=None,
            banner=None
        )  
    except:
...
async def dm(ctx, user : discord.Member, *, message): # b'\xfc'
    await ctx.message.delete()
    user = VohkesSB.get_user(user.id)
    if ctx.author.id == VohkesSB.user.id:
        return
    else:
        try:
            await user.send(message) 
...
@VohkesSB.command(name='get-color', aliases=['color', 'colour', 'sc'])
...
@VohkesSB.command(aliases=['rainbow-role'])
...
@VohkesSB.command(aliases=['slots', 'bet'])
...
@VohkesSB.command(name='auto-bump', aliases=['bump'])
async def _auto_bump(ctx, channelid): # b'\xfc'
    ...
    while True:
        try:
            count += 1 
            channel = VohkesSB.get_channel(int(channelid))
...
@VohkesSB.command(aliases=['guildpfp'])
...
@VohkesSB.command(name='backup-f', aliases=['friendbackup', 'friend-backup', 'backup-friends', 'backupf'])
async def _backup_f(ctx): # b'\xfc'
    await ctx.message.delete()
    for friend in VohkesSB.user.friends:
       friendlist = (friend.name)+'#'+(friend.discriminator)   
       with open('Backup/Friends.txt', 'a+') as f:
           f.write(friendlist+"\n" )
    for block in VohkesSB.user.blocked:
        blocklist = (block.name)+'#'+(block.discriminator)
        with open('Backup/Blocked.txt', 'a+') as f: 
            f.write(blocklist+"\n" )

@VohkesSB.command(name='first-message', aliases=['firstmsg', 'fm', 'firstmessage'])
...
@VohkesSB.command(aliases=['bitcoin'])
...
@VohkesSB.command(aliases=['ethereum'])
...
@VohkesSB.command(aliases=['wouldyourather', 'would-you-rather', 'wyrq'])
...
@VohkesSB.command(aliases=['proxy'])
...
async def purge(ctx, amount: int): # b'\xfc'
    await ctx.message.delete()
    async for message in ctx.message.channel.history(limit=amount).filter(lambda m: m.author == VohkesSB.user).map(lambda m: m):
        try:
           await message.delete()
        except:
            pass

@VohkesSB.command(name='group-leaver', aliase=['leaveallgroups', 'leavegroup', 'leavegroups'])
async def _group_leaver(ctx): # b'\xfc'
    await ctx.message.delete()
    for channel in VohkesSB.private_channels:
...
async def help(ctx): # b'\xfc'
    await ctx.message.delete()
    url = 'https://VohkesSB-selfbot.github.io/commands'
    r = requests.get(url)
    if r.status_code == 200:
        webbrowser.open(url)
    else:
...
async def stream(ctx, *, message): # b'\xfc'
    ...
    await VohkesSB.change_presence(activity=stream)    
...
async def game(ctx, *, message): # b'\xfc'
    ...
    await VohkesSB.change_presence(activity=game)

@VohkesSB.command()
async def listening(ctx, *, message): # b'\xfc'
    await ctx.message.delete()
    await VohkesSB.change_presence(
...
@VohkesSB.command(aliases=['markasread', 'ack'])
async def read(ctx): # b'\xfc'
    await ctx.message.delete()
    for guild in VohkesSB.guilds:
        await guild.ack()
...
@VohkesSB.command(name='role-hexcode', aliases=['rolecolor'])
async def _role_hexcode(ctx, *, role: discord.Role): # b'\xfc'
    await ctx.message.delete()
    await ctx.send(f"{role.name} : {role.color}")

@VohkesSB.command(name='get-hwid', aliases=['hwid', 'gethwid', 'hwidget'])
...
async def logout(ctx): # b'\xfc'
    await ctx.message.delete()
    await VohkesSB.logout()

@VohkesSB.command(aliases=['btc-stream', 'streambtc', 'stream-btc', 'btcstatus'])
async def btcstream(ctx):  # b'\xfc'
    await ctx.message.delete()   
    btc_status.start()        

@VohkesSB.command(name='steal-all-pfp', aliases=['steal-all-pfps', 'stealallpfps'])
async def _steal_all_pfp(ctx): # b'\xfc'
    await ctx.message.delete()
    Dump(ctx)

@VohkesSB.command(aliases=['clearconsole', 'consoleclear'])
...
@VohkesSB.command(name='gmail-bomb', aliases=['gmail-bomber', 'gmailbomb', 'email-bomber', 'emailbomber'])
